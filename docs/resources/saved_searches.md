...
* `action_httpalert_param_credential` - (Optional) Credential for the HTTP alert action.
* `action_httpalert_param_custom_headers` - (Optional) Custom headers for the HTTP alert action in key:value format, separated by commas.
* `action_httpalert_param_endpoint` - (Optional) Endpoint URL for the HTTP alert action.
